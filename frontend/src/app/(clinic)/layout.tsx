        ...
        <div className="border-t p-3 space-y-2">
          ...
          <div className="flex gap-1">
            {(["fr", "en"] as const).map(l => (
              <button key={l}
                ...
                aria-label={l === "fr" ? "Changer la langue en français" : "Switch language to English"}
                className={cn(
          ...
          <div className="flex items-center justify-between">
            ...
            <form action="/api/auth/logout" method="POST" style={{ display: "inline" }}>
              <button type="submit" aria-label="Se déconnecter" className="text-xs text-gray-400 hover:text-red-500 ml-2">
                Déconnexion ⎋
              </button>
            </form>
          </div>
